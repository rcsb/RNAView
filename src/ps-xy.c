                    ...
                    long *xml_ns, long *xml_bases)
{
    ...
    *num_loop1 = num_loop;
    ...
    maxdm = -XBIG;
    for (i = 1; i <= nhelix; i++)
    {                           /* determine the helix axis length */
    ...
    longest = k;
    printf("  Number of Helix =%4ld; The longest is number%3ld\n\n",
           nhelix, longest);
...
write_pdb(atmnum,AtomName,ResName,ChainID,ResSeq,xyz, Miscs,"ls.pdb");*/
    ...
    for (i = 1; i <= nhelix; i++)
    {
        ...
        xy[mm][2] = xy2[2];
        if (i == longest)
        { /* get the vector to rotate the 2D to Y axis */
...
/* get the helical axis and its head and tail points */
{
    ...
    atmnum = 1;
    for (i = 1; i <= n; i++)
    { /* get the number of atoms for the helix */
        n1 = bs_1[ii][i];
