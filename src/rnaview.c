...
int main(int argc, char *argv[])
{
    ...
    if (nxml > 10)
    { /* read information from RNAML and write into PS */
        xml2ps(inpfile, 0, 1);
        fclose(fp);
        return 0;
    }
    else if (npdb >= 10 && nxml < 10)
    {
        ...
        {
            sprintf(pdbfile_new, "%s", inpfile);
        }

        rna(pdbfile_new, type_stat, pair_stat, &base_all, chain, reso);
...
*/
{
    ...
    while (fgets(str, sizeof str, fr) != NULL)
    {
        ...
        if (!strncmp(str, "ANISOU", 6))
            continue;

        if (!strncmp(str, "ATOM  ", 6) || !strncmp(str, "HETATM", 6))
        {
            if (strlen(chain_in) > 0)
            { /*rid of chain out of range*/
...
void usage(void)
{
    ...
    printf(
        ...
        "|     Example:    rnaview  -pc --pdb ABC pdbfile_name         |\n"
        "|                                                             |\n"
        ...
        "|                                                             |\n"
        "| (7) [option] --label to process cif files using label. If   |\n"
        "|     not provided, then it will use auth.                    |\n"
        "|     Example:    rnaview -p --cif --label ciffile_name       |\n"
        "|     This will use label for parsing                         |\n"
        "|     Example:    rnaview -p --cif ciffile_name               |\n"
...
\n");
    ...
    for (i = 1; i <= nchain; i++)
    { /* rid of ligand */
        if ((chain_idx[i][2] - chain_idx[i][1]) <= 0)
            continue;
        ...
        { /*residue #*/
            ...
            if (o3p_i0 > 0 && p > 0 && o5p > 0 && c5p > 0)
                alpha = get_torsion(o3p_i0, p, o5p, c5p, xyz);
            if (p > 0 && o5p > 0 && c5p > 0 && c4p > 0)
                beta = get_torsion(p, o5p, c5p, c4p, xyz);
            if (o5p > 0 && c5p > 0 && c4p > 0 && c3p > 0)
                gamma = get_torsion(o5p, c5p, c4p, c3p, xyz);
            if (c5p > 0 && c4p > 0 && c3p > 0 && o3p > 0)
                delta = get_torsion(c5p, c4p, c3p, o3p, xyz);
            if (c4p > 0 && c3p > 0 && o3p > 0 && p_i1 > 0)
                epsilon = get_torsion(c4p, c3p, o3p, p_i1, xyz);
            if (c3p > 0 && o3p > 0 && p_i1 > 0 && o5p_i1 > 0)
                zeta = get_torsion(c3p, o3p, p_i1, o5p_i1, xyz);

            if (o4p > 0 && c1p > 0 && n9 > 0 && c2 > 0)
                chi = get_torsion(o4p, c1p, n9, c2, xyz);
...
/* do all sorts of calculations */
{
    ...
    else
    {
        ...
        */
        ...
        if (reso_in > 0.1 && strlen(strReso))
        {
            if (sscanf(strReso, "%f", &reso) == 1)
            {
                if (reso > reso_in)
                {
                    num = 0;
        ...
        for (index_uf = 0; index_uf < num; index_uf++)
        {
            ...
            if (AtomName_uf[index_uf][strlen(AtomName_uf[index_uf]) == 4] == 'H')
            ...
            if (!strncmp(Group_uf[index_uf], "ANISOU", 6))
            {
                continue;
            }

            if (!strncmp(Group_uf[index_uf], "ATOM", 4) || !strncmp(Group_uf[index_uf], "HETATM", 6))
            {
                if (strlen(chain_in) > 0)
                { /*rid of chain out of range*/
                    if (!strchr(chain_in, ChainID_uf[index_uf][0]))
                    {
                        continue;
                    }
                }

                char *resname = ResName_uf[index_uf];

                if (resname[0] == 'D' && strlen(resname) == 2)
                {
                    if (resname[1] == 'A' || resname[1] == 'T' || resname[1] == 'G' || resname[1] == 'C')
                    {
                        ResName_uf[index_uf][0] = ResName_uf[index_uf][1];
                        ResName_uf[index_uf][1] = '\0';
                    }
                }
            }
            ...
            int atomNameShift = atomNameLength < 4;
            int resNameShift = 3 - resNameLength;

            strcpy(AtomName[curr_index], "    ");
            strcpy(ResName[curr_index], "   ");

            strncpy(AtomName[curr_index] + atomNameShift, AtomName_uf[index_uf], atomNameLength);
            strncpy(ResName[curr_index] + resNameShift, ResName_uf[index_uf], resNameLength);
...

{
    ...
    */
    ...
    if (!num_pair_tot)
    {
        ...
        fclose(fout);
        if (PS > 0)
            printf("No 2D structure plotted!\n");
        return;
    }
    ...
    */
    ...
    while (fgets(str, sizeof str, finp) != NULL)
    {
        if (strstr(str, "BEGIN_base-pair"))
        {
            ...
            while (fgets(str, sizeof str, finp) != NULL)
            {
                if (strstr(str, "!") || strstr(str, "stack"))
                    continue;
    ...
    for (i = 0; i < np; i++)
    {
        if ((strstr(str_pair[i], "+/+") || strstr(str_pair[i], "-/- ")) && strstr(str_pair[i], "cis "))
        {
