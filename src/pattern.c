...

{
    ...
    { /* get # of lines */
        if (strstr(str, "BEGIN_base-"))
        {
            ...
            while (fgets(str, sizeof str, finp) != NULL)
            {
                if (strstr(str, "stacked"))
                    continue; /*do not show stack case*/
...
/* match the patterns given by user */
{
    ...
    ng = 0;
    while (fgets(str, sizeof str, patt_in) != NULL)
    { /* get the input type*/
        if (strchr(str, '#') || strlen(str) <= 0)
            continue;
    ...
    for (j = 0; j < npatt; j++)
    {
        n = group_idx[j] - 2;
        if (ng == 1 || ng == 2)
        { /* for short pattern, ask exact match */
            if (n != ng)
                continue;
        ...
        { /* possible 3, 4,5,6,7,8 (3 4 4 4 4 5 ) */
            if (n > ng + 3 || n < ng - 2)
                continue;
            get_patt(j, ng, group_idx, type, user_type, &nm);
        ...
        { /* possible 4,5,6,7,8 ,9 (4 5 5 5  6 6) */
            if (n > ng + 3 || n < ng - 2)
                continue;
            get_patt(j, ng, group_idx, type, user_type, &nm);
...

{
    ...
    *yes = 0;
    ...
    if (group_idx[n1] <= 4 && group_idx[n2] != group_idx[n1])
        return;
