    ...
    { /* get # of lines */
        if (strstr(str, "BEGIN_base-pair"))
        {
            while (fgets(str, sizeof str, finp) != NULL)
            {
                ...
                fprintf(xml, "%s<molecule-id ref=\"%ld\"/><position>%ld</position>\n",
                  ...
                  char *parfile, long *sugar_syn, FILE *xml)
{
    ...
    fprintf(xml, "%s<molecule id=\"%ld\">\n", tag1, molID);

    fprintf(xml, "%s<sequence>\n", tag2);

    fprintf(xml, "%s<numbering-system id=\"%ld\" used-in-file=\"false\">\n", tag3, molID);
    fprintf(xml, "%s<numbering-range>\n", tag4);
    fprintf(xml, "%s<start>%ld</start>\n", tag5, POSITION_1_N[start]);
    fprintf(xml, "%s<end>%ld</end>\n", tag5, POSITION_1_N[end]);
    fprintf(xml, "%s</numbering-range>\n", tag4);
    fprintf(xml, "%s</numbering-system>\n", tag3);

    res_length = end - start + 1;
    fprintf(xml, "%s<numbering-table length=\"%ld\" comment=\"sequence number in pdb file\">\n", tag3, res_length);
    fprintf(xml, "%s", tag3);

    for (i = start; i <= end; i++)
    {
        fprintf(xml, "%4ld ", ResSeq[seidx[i][1]]);
    ...
    if (num_loop > 0)
    {
        ...
        {
            if (chain_nam != ChainID[seidx[loop[i][1]][1]])
                continue;
