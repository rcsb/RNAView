...
/* Account only the real anti-parrellel helix from the longer  helix.*/
{
    ...
    nregion = 1;
    for (i = 1; i <= n; i++)
    { /* get the anti-parrellel helix region */
        helix[nregion][1] = i;
        for (i++; (i <= n) &&
                  (((bs_1[j][i] == bs_1[j][i - 1] + 1) && (bs_2[j][i] == bs_2[j][i - 1] - 1)) ||
                   ((bs_2[j][i] == bs_2[j][i - 1] + 1) && (bs_1[j][i] == bs_1[j][i - 1] - 1)));
             i++)
        {
            helix[nregion][2] = i;
        }
        i--;
        if (helix[nregion][2] - helix[nregion][1] >= 1)
        { /* minimum two pairs */
            nregion++;
        }
    }
 ...
 n1 increases and n2 decrease. */
{
    ...
    *yes = 0;
    ...
    if (n == n2 - n1 - 1 && ChainID[k1] == ChainID[k2])
    { /* a possible loop */
        *yes = 1;
        do
        {
