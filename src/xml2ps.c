...
/*int main() */
{
    ...
    long i, j, k, n, nxy, *resid_idx, nchain, **chain_idx, *author_seq;
...
if(nres<=80)
    ...
    */
    ...
    if (nxy != nres || nxy != npo3 || nres != npo3)
        printf("# xy: # of bases: # of O3' %ld %ld %ld\n", nxy, nres, npo3);
    ...
    */
    ...
    printf("Number of base pairs (including tertiary interactions):   %ld\n", npair);
    printf("Number of helix (anti-parallel):   %ld\n", nhelix);
    printf("Number of single strand:   %ld\n", nsing);
...
*/
{
    ...
    { /* start from the second */
        ...
        if (dis > 2.2 || i == nres - 1)
        { /* chain broken; renumber the chain*/
            chain_broken[i] = 1;
            j++;
        }
        /*
        ...
        if (strcmp(item, "MOLECULE") == 0)
        {
            do
            {
                ...
                if (!strcmp(item, "STRUCTURE"))
                {
                    ...
                    {
                        element_in_bracket(inp, item, &size, lett, &key);
                        if (key == 1 || !strcmp(item, "/STRUCTURE"))
                            break;
...
/* get the position number */
{
    ...
    { /* get the letters before the first > */
        if (ch == EOF)
            return;
        {
            letters[n] = ch;
    ...
    k = 0;
    for (j = 0; j < n; j++)
    { /* get the letters before the first space */
        if (letters[j] == ' ')
        {
            m = j;
            break;
    ...
    k = 0;
    for (j = m + 1; j < n; j++)
    { /* get the letters after the first space */
        if (letters[j] == ' ')
        {
            m = j;
            break;
                    ...
                    long *nsing, long *sing_st, long *sing_end)
{
    ...
    while (1)
    {
                            ...
                            do
                            {
                                ...
                                if (!strcmp(item, "STR-ANNOTATION"))
                                {
                                    do
                                    {
                                        ...
                                        else if (!strcmp(item, "SINGLE-STRAND"))
                                        { /*for single strand */
                                            do
                                            {
                                                element_in_bracket(inp, item, &size, lett, &key);
        ...
        else if (strcmp(item, "INTERACTIONS") == 0)
        {
            ...
            {
                element_in_bracket(inp, item, &size, lett, &key);
                if (key == 1 || !strcmp(item, "/INTERACTION"))
                    break;
                if (!strcmp(item, "STR-ANNOTATION"))
                {
                    do
                                    ...
                                    do
                                    {
                                        ...
                                        if (!strcmp(item, "/SECONDARY-STRUCTURE-DISPLAY"))
                                        {
                                            *num_xy = nxy;
                                            ...
                                            if (nxy != nposit)
                                                printf("Warnning! nxy (%ld) not equal to nposit (%ld)\n", nxy, nposit);
                                        }
            ...
            do
            {
                ...
                { /* for identity */
                    do
                    {
                        ...
                        {
                            if (nseq != *nres)
                                printf("Warnning! author seqence (%ld) not equal to residue number (%ld)\n", nseq, *nres);
                            break;
                        }
...
*/
{
    ...
    *size = n;
    for (i = 0; i < n; ++i)
    { /* get the identifier for the first word */
        if (lett[i] == ' ')
            break;
...
void link_chain(long nchain, long **chain_idx, double **xy, long *chain_broken)
{
    ...
    for (i = 0; i < nchain; i++)
    {
        ...
        if ((chain_idx[i][1] - chain_idx[i][0]) < 1)
            continue;
        ...
        for (j = chain_idx[i][0]; j < chain_idx[i][1]; j++)
        { /*link chain*/
            if (j < last && chain_broken[j + 1] == 1)
                continue;
    ...
    { /* label single strand */
        ...
        printf("single_xml2ps  %4d  %4d (%4d)\n", i, j, author_seq[j]);
            */
            ...
            if (sign >= 0)
            {
                fprintf(psfile, "gsave Tl %.2f %.2f moveto (%ld) center show grestore\n",
...
/* reset x/y coordinates to PS units */
{
    ...
    if (frame_box)
    {
        ...
        fprintf(psfile, format, urxy[1] + boundary_offset, urxy[2] + boundary_offset);
