                      ...
                      long **sugar, long **chi, double **xyz, FILE *fp)
{
    ...
    for (i = 1; i <= ds; i++)
    {
        for (j = 1; j <= num_bp; j++)
        {
            ...
            {
                P_angle =
                    atan2(sugar_angle[i][o7 + 5] + sugar_angle[i][o7 + 2] - sugar_angle[i][o7 + 4] - sugar_angle[i][o7 + 1],
   ...
   B- or TA-like */
{
    ...
    if (mtwist > 10.0 && mtwist < 60.0)
    { /* right handed DNA/RNA */
        print_sep(fp, '*', 76);
        ...
        for (i = 1; i <= nbpm1; i++)
        {
            ...
            if (phos[1][i + 1] && phos[2][i])
            {
                ...
                if (WC_info[i] && WC_info[i + 1] && !bphlx[i] /* WC & non-break */
                    ...
                    && aveS[i][1] > -5.0 && aveS[i][1] < -0.5 && aveS[i][2] > 7.5 && aveS[i][2] < 10.0 && aveS[i][3] > -2.0 && aveS[i][3] < 3.5 && aveH[i][1] > -11.5 && aveH[i][1] < 2.5 && aveH[i][2] > 1.5 && aveH[i][2] < 10.0 && aveH[i][3] > -3.0 && aveH[i][3] < 9.0)
                {                          /* normal limits */
   ...
   measuring curvature, and bending angle */
{
    ...
    fprintf(fp, "      P: %.2f(%.2f), O4': %.2f(%.2f),  C1': %.2f(%.2f)\n",
            ...
            std_dvector(o4_radius, no4), rave[2], std_dvector(c1_radius, nc1));
