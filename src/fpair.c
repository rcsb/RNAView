        ...
        for (j = i + 1; j <= num_residue; j++)
        {
            ...
            a key wheather to include O3', O5', O1P,O2P;   0 not include
              ...
              */
                ...
                { /* more than 2 H bonds (into more details)*/
...
/* get the coordinate of ring center*/
{
    ...
    if (bseq[i] == 'A' || bseq[i] == 'G' || bseq[i] == 'a' || bseq[i] == 'g' || bseq[i] == 'I' || bseq[i] == 'i')
    {
        natm = 9;
    }
 ...
 */
{
    ...
    *bpid = 0; /* default as not-paired */
    ...
    if (rtn_val[3] > BPRS[4])
        return;
    if (rtn_val[3] <= 10 && rtn_val[2] >= 2.2)
        return; /*   may be removed later!! */
    ...
    */
    ...
    if (network)
    { /* check if two bases in pairing network */
    ...
    */
    ...
    if (rtn_val[4] <= 6 && rtn_val[2] >= 1.80)
        return; /*further restrain */

    if (rtn_val[1] <= BPRS[2] && rtn_val[2] <= BPRS[3] && rtn_val[3] <= BPRS[4] && rtn_val[4] >= BPRS[5])
    {
            ...
            for (n = seidx[j][1]; n <= seidx[j][2] && !short_contact; n++)
            {
                ...
                if (strchr("ON", AtomName[m][1]) /*only base - base */
                   ...
                   */
                    ...
                    && strchr("ON", AtomName[n][1]) && AtomName[n][0] == ' ' && isdigit(AtomName[n][2])
                    /*
                    && (AtomName[n][3] == ' ' )
                      */
                )
        ...
        if (short_contact)
        {
            ...
            base_stack(i, j, bseq, seidx, AtomName, xyz, rtn_val, &stack_key);
            if (stack_key > 0)
                return; /* rid of base-base stack cases */
            ...
            *bpid = -1; /* assumed to be non-WC */
            ...
            for (k = 1; k <= 3; k++)
            {
                ...
                for (l = 1; l <= 3; l++)
                {
                    ...
                    r2[l][k] = (k == 1) ? orien[j][koffset + l] : -orien[j][koffset + l];
                }
            }
    ...
    *yes = 0;
    ...
    for (m = seidx[i][1]; m <= seidx[i][2]; m++)
    {
        ...
        for (n = seidx[j][1]; n <= seidx[j][2]; n++)
        {
            ...
            if (dd <= hlimit && !strchr(AtomName[m], '\'') && !strchr(AtomName[n], '\''))
            {
                *yes = 1;
    ...
    */
    ...
    for (ii = 1; ii <= num_helixs; ii++)
    {
        ...
        for (i = helix_idx[ii][1]; i <= helix_idx[ii][2]; i++)
        {
            ...
            fprintf(fout1, "%5ld%5ld%5ld %ld%4ld| %s-%s-%s", num_bsp, i_order,
 ...
 */
{
    ...
    *bpid = 0; /* default as not-paired */
    ...
    if (j == i + 1 && rtn_val[2] >= 2.0)
        return; /* constrain for the neighbor */

    rtn_val[5] = rtn_val[1] + 2.0 * rtn_val[2];

    if (network)
    { /* check if two bases in pairing network (not origins!) */
        if (rtn_val[2] <= BPRS[3] && rtn_val[3] <= BPRS[4] && rtn_val[4] >= BPRS[5])
            *bpid = 1;
        return;
    }
    ...
    if (rtn_val[1] <= BPRS[2])
    {
        ...
        for (m = seidx[i][1]; m <= seidx[i][2] && !short_contact; m++)
        {
            ...
            for (n = seidx[j][1]; n <= seidx[j][2] && !short_contact; n++)
                if (strchr("ON", AtomName[m][1]) && strchr("ON", AtomName[n][1]) && AtomName[m][0] == ' ' && isdigit(AtomName[m][2]) && AtomName[m][3] == ' ' /* base atom on residue m */
                    && AtomName[n][0] == ' ' && isdigit(AtomName[n][2]) && AtomName[n][3] == ' ')
                {
...
/* this is filter for NO which can not form H bond (base-base) */
{
    ...
    for (m = seidx[i][1]; m <= seidx[i][2]; m++)
    {
        ...
        if (toupper(bseq[i]) == 'A' || toupper(bseq[i]) == 'I')
        { /*filter */
        ...
        for (n = seidx[j][1]; n <= seidx[j][2]; n++)
        {
            ...
            if (toupper(bseq[j]) == 'A' || toupper(bseq[j]) == 'I')
            { /*filter */
            ...
            else if ((AtomName[m][3] == 'P' && AtomName[n][3] != '\'' && AtomName[n][1] != 'C') ||
                     (AtomName[n][3] == 'P' && AtomName[m][3] != '\'' && AtomName[m][1] != 'C'))
            {
