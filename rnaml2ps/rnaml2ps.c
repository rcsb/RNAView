...
/* int main()*/
{
    ...
    int nmol,  resid_num=5000;
    
    long nres=0,  npair, num_pair, **npair_idx, *sugar_syn;
    ...
    if(argc==1 || (argc>1 && !strcmp(argv[1], "-h"))){
        printf("Usage: excutable   RNAML_file_name\n");
        exit(0);
    }

    strcpy(inpfile, argv[1]);
    sprintf(outfile, "%s.ps",inpfile);
    ...
    chain_idx = lmatrix(0, 100, 0 ,2);  /*maximum 100 chain */
...
/*
    printf("Number of xy coordinates: # of bases: %d %d\n", nxy, nres);
*/
    for(i=0; i< nxy; i++){ 
        xy0[i][0] = xy[i][0];  
        xy0[i][1] = xy[i][1];
...
*/    
    ...
    nchain=NMOL;
    link_chain(nchain, chain_idx, xy);
...
*/
    ...
    free_lmatrix(chain_idx , 0, 100, 0 ,2);  
...
/*	printf("dist %4d %8.2f\n", i, dis); */
        if(dis>2.0 || i==nres-1) { /* chain broken; renumber the chain*/
            chain_idx[j][1] =  i-1;
            if(i==nres-1) chain_idx[j][1] =  i;
            j++;
            chain_idx[j][0] = i;
            if(j>nres-4){
...
void read_O3prime_P_xyz(char *inpfile, double **o3_prime_xyz, double **p_xyz)
...
void link_chain(long nchain,long **chain_idx,  double **xy)
{
...
{
    long n,fill, width=1;
    double xy0[3], a, d,  d1, d2, d3, dpair, r;
    ...
    */
    ...
    twopoints(xy0, at, d1, xy1, xy6);            /*  1-2..3-4..5-6  */
